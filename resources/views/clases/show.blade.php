    ...
    <div class="max-w-4xl mx-auto p-6">
        <div class="bg-white dark:bg-gray-800 rounded shadow p-6 border-l-4"
            ...
            <ul class="text-gray-700 dark:text-gray-300 space-y-2 text-sm sm:text-base">
                ...
                @if($clase->url_zoom)
                    <li><strong>Enlace Zoom:</strong> <a href="{{ $clase->url_zoom }}" target="_blank" class="text-blue-500 underline">{{ $clase->url_zoom }}</a></li>
                @endif
            </ul>

            <div class="flex gap-3 mt-6">
                <a href="{{ route('clases.edit', $clase) }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded">✏️ Editar</a>
                <form action="{{ route('clases.destroy', $clase) }}" method="POST" onsubmit="return confirm('¿Eliminar esta clase?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded">🗑️ Eliminar</button>
                </form>
                <a href="{{ route('clases.index') }}" class="px-4 py-2 bg-gray-300 dark:bg-gray-600 text-sm rounded text-gray-800 dark:text-white">← Volver</a>
            </div>
        </div>
    </div>
</x-app-layout>
