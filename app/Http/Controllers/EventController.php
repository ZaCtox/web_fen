...
public function index()
{
    ...
    // Eventos automáticos generados por uso de salas
    $classEvents = collect();
    $dias = [
        'Domingo' => 0,
        'Lunes' => 1,
    ...
    RoomUsage::with(['room', 'period', 'course'])->get()->each(function ($uso) use (&$classEvents, $dias) {
        if (
            !$uso->period ||
            !$uso->course ||
            !$uso->hora_inicio ||
        ...
        $fechaInicio = \Carbon\Carbon::parse($uso->period->fecha_inicio);
        $fechaFin = \Carbon\Carbon::parse($uso->period->fecha_fin);
        ...
        while ($fechaInicio->lte($fechaFin)) {
            ...
            $classEvents->push([
                ...
                'description' => 'Magíster: ' . $uso->course->programa,
    ...
    // Unimos eventos manuales y automáticos
