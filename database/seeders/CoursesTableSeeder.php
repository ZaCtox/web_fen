<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use App\Models\Course;
use App\Models\Magister;

class CoursesTableSeeder extends Seeder
{
    public function run()
    {
        $data = [
            'Economía' => [
                "Econometría I",
                "Econometría II",
                "Macroeconomía Avanzada I",
                "Macroeconomía Avanzada II",
                "Matemáticas Avanzadas para Economía",
                "Metodología de Investigación en Economía",
                "Microeconomía Avanzada I",
                "Microeconomía Avanzada II",
                "Microeconomía Avanzada III",
                "Proyecto de Tesis",
                "Tesis"
            ],
            'Dirección y Planificación Tributaria' => [
                "Administración",
                "Administración Financiera",
                "Administración Tributaria Comparada",
                "Aspectos legales de la Empresa",
                "Contabilidad y Tributación",
                "Electivo I",
                "Electivo II",
                "Empresa y su Compromiso Tributario",
                "Entorno Económico",
                "Finanzas Internacionales",
                "Finanzas Públicas",
                "Fondo de Utilidades Tributarias",
                "Franquicias Tributarias",
                "Impuesto Global Complementario",
                "Impuesto al Valor Agregado Aplicado",
                "Impuestos Directos en Empresas",
                "Lo Contencioso Tributario",
                "Mercado de Capitales y su Tributación",
                "Planificación Tributaria",
                "Taller 1 - Habilidades de Aprendizaje: Presentación Efectiva, Trabajo en Equipo, Metodología de Casos",
                "Trabajo de Grado I",
                "Trabajo de Grado II",
                "Trabajo de Grado III",
                "Tributación Internacional",
                "Tributación de Rentas del Trabajo",
                "Ética y Tributación"
            ],
            'Gestión de Sistemas de Salud' => [
                "Aspectos Legales en Salud",
                "Calidad y Acreditación en Salud",
                "Control Estratégico de Instituciones de Salud",
                "Desarrollo de Competencia Relaciónales",
                "Dirección Estratégica de Recursos Humanos",
                "Dirección Estratégica en Sistemas de Salud",
                "Epidemiología y Salud Pública para la Gestión",
                "Estadística para la gestión",
                "Formulación y Evaluación de Proyectos en Salud",
                "Gestión de Operaciones, Logística y Calidad",
                "Sistema de Salud y Gestión en Red",
                "Taller 1 - Habilidad e Aprendizaje: Presentación efectiva, Trabajo en Equipo, Metodología de Casos",
                "Taller 2: Herramientas para el trabajo de grado: Métodos y técnicas para la investigación en gestión",
                "Taller 3: Desarrollo y crecimiento personal"
            ],
            'Gestión y Políticas Públicas' => [
                "Cambio Organizacional y Gestión de Personas",
                "Contabilidad",
                "Contabilidad IFRS",
                "Control de Gestión",
                "Dirección Estratégica",
                "Economía",
                "Entorno Normativo Institucional del Sector Público",
                "Entorno Socio Cultural",
                "Entorno Territorial",
                "Estadística para la Gestión",
                "Evaluación de Programas y Proyectos Públicos",
                "Gestión Pública I",
                "Gestión Pública II",
                "Gestión Pública III",
                "Introducción a la Ciencia Política y Entorno Político",
                "Políticas Públicas II",
                "Regulación y Políticas Públicas I",
                "Taller 1 - Habilidades de Aprendizaje: Presentación efectiva, Trabajo en equipo, Metodología de Casos",
                "Taller 2 - Herramientas para el Trabajo Grado: Métodos y Técnicas para la Investigación en Gestión"
            ]
        ];

        foreach ($data as $magisterNombre => $cursos) {
            $magister = Magister::where('nombre', trim($magisterNombre))->first();

            if (!$magister) {
                throw new \Exception("Magíster no encontrado: '$magisterNombre'. Asegúrate de que el seeder de Magisters se ejecutó correctamente y que el nombre coincide exactamente.");
            }

            foreach ($cursos as $curso) {
                Course::create([
                    'nombre' => $curso,
                    'magister_id' => $magister->id,
                ]);
            }
        }
    }
}
